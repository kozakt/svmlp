...
local function cleanBlock(block)
	...
	if block.filename then
		block.filename = nil
		local basedir = block.basedir
		if basedir then
			basedir = tostring(basedir):lower()
		end
...
local function findImportsInBlock(self, block)
	if block.import then
		...
		for _, entry in ipairs(block.import) do
			...
			-- iter the sorted table
			for prjName, filters in pairsByKeys(entry) do
  ...
  local parsed = self.parsedImports[prjName]
	...
	for exp in p.container.eachChild(prj, p.export) do
		...
		if not table.contains(parsed, exp.name) then
			local preparedFilters = prepareFilters(filters)

			-- Only import if the filters matches
			if matchFilters(exp.name, preparedFilters) then
